...
```shell-session
frodonomojo@htb[/htb]$ gobuster dir -u http://10.10.10.121/ -w /usr/share/dirb/wordlists/common.txt
...
```shell-session
frodonomojo@htb[/htb]$ git clone https://github.com/danielmiessler/SecLists
```

  Web Enumeration

```shell-session
frodonomojo@htb[/htb]$ sudo apt install seclists -y
```
...
```shell-session
frodonomojo@htb[/htb]$ gobuster dns -d inlanefreight.com -w /usr/share/SecLists/Discovery/DNS/namelist.txt
...
```shell-session
frodonomojo@htb[/htb]$ curl -IL https://www.inlanefreight.com
...
```shell-session
frodonomojo@htb[/htb]$ whatweb 10.10.10.121
...
```shell-session
frodonomojo@htb[/htb]$ whatweb --no-errors 10.10.10.0/24
...
**N.B.:** During the OSCP, utilize /usr/share/wordlists/seclists/Discovery/Web-Content/common.txt to find subdirectories on the website. Also use the dir directory wordlist if the dirbuster directory wordlist produces few results as well.
