...
```shell-session
frodonomojo@htb[/htb]$ cd /tmp
frodonomojo@htb[/htb]$ python3 -m http.server 8000
...
```shell-session
frodonomojo@htb[/htb]$ scp linenum.sh user@remotehost:/tmp/linenum.sh
...
```shell-session
frodonomojo@htb[/htb]$ base64 shell -w 0
...
```shell-session
frodonomojo@htb[/htb]$ md5sum shell
